...
function Login() {
  ...
  // Handle button clicks, limit input to 8 digits
  const handleButtonClick = (value) => {
    setInputValue((prev) => (prev.length < 8 ? prev + value : prev)); // Append only if length is less than 8
  };
  ...
  // Handle back button click to navigate to root
  const handleBack = () => {
    navigate('/');
  };
